...
import itv.model.exceptions.ClienteNoEncontradoException;
import itv.model.exceptions.ClienteSinVehiculosException;
import itv.model.exceptions.HorarioNoDisponibleException;
import itv.model.exceptions.VehiculoNoEncontradoException;
...
private boolean matriculaYaExiste(String matricula) {
...
private void registrarInspeccion(Scanner scanner) {
    ...
    try {
        ...
        if (cliente.getVehiculos().isEmpty()) {
            throw new ClienteSinVehiculosException("El cliente no tiene vehiculos registrados.");
        }
        ...
        if (opcionVehiculo < 0 || opcionVehiculo >= cliente.getVehiculos().size()) {
            throw new VehiculoNoEncontradoException("El vehiculo seleccionado no existe.");
        }
        ...
        if (opcionInspector < 0 || opcionInspector >= inspectores.size()) {
            System.out.println(" Opcion de inspector invalida.");
            return;
        }
        ...
        if (calendarioInspecciones[opcionDia][opcionHora] != null) {
            throw new HorarioNoDisponibleException("El horario seleccionado ya esta ocupado.");
        }
        ...
        System.out.println(" Inspeccion programada el " + DIAS_SEMANA[opcionDia] + " a las " + HORARIOS[opcionHora]
                + " con el inspector " + inspector.getNombre() + " para el vehiculo " + vehiculo.getMatricula());

        
    } catch (ClienteNoEncontradoException e) {
        System.out.println(" Error: " + e.getMessage());
    } catch (ClienteSinVehiculosException e) {
        System.out.println(" Error: " + e.getMessage());
